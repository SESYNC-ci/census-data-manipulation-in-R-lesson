...
To complete the pivot table, however, the `species_month_prop` values must be copied into a R data.frame using `collect` for processing with `pivot_wider`. There is no SQL equivalent for what `pivot_wider` does.

```{r handout = 0}
pivot <- species_month_prop %>%
  collect() %>%
  pivot_wider(month, prop, values_fill = list(prop = 0))
```
