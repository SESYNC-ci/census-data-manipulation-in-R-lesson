...
cbp15 <- read.csv('data/cbp15co.txt') %>%
...
# Make a new variable that combines state and county fips by combining and padding the
cbp <- cbp15_2 %>%
  mutate(chrFIPSCTY = substr(paste("00", as.character(FIPSCTY), sep = ""), nchar(as.character(FIPSCTY)), nchar(paste("00", as.character(FIPSCTY), sep = "")))) %>%
  mutate(fips = as.numeric(paste(as.character(FIPSTATE), chrFIPSCTY, sep = ""))) %>%
  select(c(fips, naics, est=EST)) # and drop/rename vars
...
# Create data set of county names to merge 

countyNames <- read.csv("data/national_county.txt") %>%
  mutate(COUNTY_ST = paste(COUNTY, STATE, sep = ", ")) %>%
  mutate(chrFIPSCTY = substr(paste("00", as.character(FIPSCTY), sep = ""), nchar(as.character(FIPSCTY)), nchar(paste("00", as.character(FIPSCTY), sep = "")))) %>%
  mutate(combinedFIPS = as.numeric(paste(as.character(FIPSTATE), chrFIPSCTY, sep = ""))) %>%
  select(fips=combinedFIPS, county=COUNTY_ST)
